...
document.addEventListener('DOMContentLoaded', (event) => {
  ...
  img.onload = function() {
    ...
    const palette = colorThief.getPalette(img).map((x) => `rgb(${x.join(',')})`)
...
const postLoad = () => {
  ...
  const topPath = `M ${cx} ${cy} C ${cx},${cy - offset} ${endX},${cy - offset} ${endX} ${cy}`
  document.getElementById('top').setAttribute('d', topPath)

  let bottomPath = `M ${cx} ${cy} C ${cx},${cy + offset } ${endX},${cy + offset} ${endX} ${cy}`
